...
Webシステムおよびアプリの要件を整理し、要件定義ドキュメントを生成します。
...
### 2. 機能要件の整理
以下のカテゴリで分類して記述する:
- **Webシステム（共通基盤）**: ユーザー認証・セッション管理・アプリ管理・共通API・共通UI部品
- **アプリプラグイン機構**: manifest.json 読み込み・有効化/無効化・独立データ領域
- **各アプリ**: アプリ個別の機能（対象アプリが決まっていれば記述）
...
- DO NOT `documents/01-requirements/` 以外のファイルを編集しない
